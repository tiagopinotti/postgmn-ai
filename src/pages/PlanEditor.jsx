...
import { callAI, buildWhatsAppLink, extractJSON } from '../lib/ai.js'
...
const MODELS = {
  ...
  gemini: ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-1.5-pro-latest'],
...
export default function PlanEditor() {
  ...
  const [aiProvider, setAiProvider] = useState(() => localStorage.getItem('PREF_AI_PROVIDER') || 'claude')
  const [aiModel, setAiModel] = useState(() => localStorage.getItem('PREF_AI_MODEL') || MODELS[localStorage.getItem('PREF_AI_PROVIDER') || 'claude'][0])
  const [genWithImage, setGenWithImage] = useState(false)
  const [showSettings, setShowSettings] = useState(false)
  const [configTick, setConfigTick] = useState(0)
  const imageProvider = localStorage.getItem('API_KEY_FREEPIK') ? 'freepik' : (aiProvider === 'gemini' ? 'gemini' : 'openai')
  ...
  async function loadData() {
    const [
      ...
      { data: kws }, { data: svcs }, { data: al }, { data: vg }, { data: tpls },
    ] = await Promise.all([
      ...
      supabase.from('approval_links').select('token').eq('content_plan_id', planId).eq('is_active', true).gte('expires_at', new Date().toISOString()).order('created_at', { ascending: false }).limit(1),
      supabase.from('client_visual_guidelines').select('*').eq('client_id', clientId).single(),
      supabase.from('post_templates').select('*').eq('client_id', clientId).order('sort_order'),
    ])
    setClient(c); setPlan(p); setPosts(ps || [])
    setKeywords(kws || []); setServices(svcs || [])
    setVisualGuide(vg || null)
    setDbTemplates(tpls || [])
...
${client.strategy_notes ? `- Contexto especial do cliente para este período: ${client.strategy_notes}` : ''}
${visualGuide ? `- Identidade visual: estilo ${visualGuide.visual_style || 'moderno'}, cores da marca: ${(visualGuide.color_palette || []).join(', ') || 'não definidas'}, tipografia: ${visualGuide.font_notes || 'padrão'}${visualGuide.notes ? `, observações: ${visualGuide.notes}` : ''}` : ''}
Regras:
- Responder em PT-BR
...
${dbTemplates.length > 0 ? `- Templates de arte disponíveis (use o campo template_tag para indicar qual template combina com cada post):
${dbTemplates.map(t => `  * "${t.name}" (tags: ${(t.tags || []).join(', ')})`).join('\n')}
  * Se nenhum template combinar, use "default"` : ''}
- Retornar SOMENTE o JSON, sem texto antes ou depois
Formato esperado (array JSON):
[{"week":1,"suggested_date":"${plan.year}-${String(plan.month).padStart(2,'0')}-05","theme":"Nome do tema","category":"Educativo","goal":"objetivo do post"${dbTemplates.length > 0 ? ',"template_tag":"nome do template"' : ''}}]`

      const text = await callAI(prompt, aiProvider, aiModel)
      ...
      const { data } = await supabase.from('posts').insert(items.map(item => {
        // Match template_tag to a DB template
        let matchedTemplate = null
        if (item.template_tag && dbTemplates.length > 0) {
          matchedTemplate = dbTemplates.find(t => 
            t.name.toLowerCase() === item.template_tag.toLowerCase() ||
            (t.tags || []).some(tag => tag.toLowerCase() === item.template_tag.toLowerCase())
          )
          if (!matchedTemplate) matchedTemplate = dbTemplates.find(t => t.is_default)
        }
        return {
          ...
          image_template: matchedTemplate?.id || null,
        }
      })).select()
      setPosts(data || [])
  ...
  async function generatePostContent(post) {
    ...
    try {
      const prompts = [
        callAI(`Você é um redator especialista em Google Meu Negócio.
...
${client.strategy_notes ? `- Contexto especial: ${client.strategy_notes}` : ''}
${visualGuide ? `- Identidade visual da marca: estilo ${visualGuide.visual_style || 'moderno'}${visualGuide.notes ? `, ${visualGuide.notes}` : ''}` : ''}
Regras: texto claro, natural, estruturado em parágrafos curtos (use quebras de linha \\n\\n), pensado para Google Meu Negócio, inclua CTA sutil para WhatsApp, máximo de 900 caracteres.
Retorne SOMENTE o JSON: {"internal_title":"","post_text":""}`, aiProvider, aiModel),
        callAI(`Crie uma mensagem curta de WhatsApp (máximo 3 linhas) como se fosse um cliente que viu o post no Google.
Empresa: ${client.company_name}, Tema: ${post.theme}, Serviço: ${services[0]?.name || client.niche || ''}, CTA: ${client.main_cta || 'entre em contato'}
Retorne SOMENTE o JSON: {"whatsapp_message":""}`, aiProvider, aiModel)
      ]

      if (genWithImage) {
        const visualCtx = visualGuide ? ` Style: ${visualGuide.visual_style || 'modern'}, Colors: ${(visualGuide.color_palette || []).join(', ')}, ${visualGuide.notes || ''}` : ''
        prompts.push(generateAIImage(`Image for GMB post about ${post.theme}.${visualCtx}`, {
      ...
      const postText = results[0]
      const waMsg = results[1]
      const imageUrl = genWithImage ? results[2] : null
      ...
      const updates = {
        ...
        image_prompt: genWithImage ? `Image for GMB post about ${post.theme}` : post.image_prompt
      }
  ...
  async function generatePostImage(post) {
    ...
    setGeneratingImage(post.id); setAiError('')
    try {
      const visualCtx = visualGuide ? ` Style: ${visualGuide.visual_style || 'modern'}, Colors: ${(visualGuide.color_palette || []).join(', ')}, ${visualGuide.notes || ''}` : ''
      const imageUrl = await generateAIImage(`Image for GMB post about ${post.theme}.${visualCtx}`, {
        bizName: client.company_name,
        theme: post.theme,
        provider: imageProvider
      })
      const updates = { image_url: imageUrl, image_prompt: `Image for GMB post about ${post.theme}` }
      await supabase.from('posts').update(updates).eq('id', post.id)
      setPosts(prev => prev.map(p => p.id === post.id ? { ...p, ...updates } : p))
      if (selectedPost?.id === post.id) setSelectedPost(p => ({ ...p, ...updates }))
    } catch(e) { setAiError(e.message) }
  ...
  // configTick forces re-calculation when settings are saved
  const noApiKey = ((tick) => {
    if (aiProvider === 'claude') return !(localStorage.getItem('API_KEY_CLAUDE') || import.meta.env.VITE_ANTHROPIC_KEY)
    if (aiProvider === 'gemini') return !(localStorage.getItem('API_KEY_GEMINI') || import.meta.env.VITE_GOOGLE_AI_KEY)
    if (aiProvider === 'openai') return !(localStorage.getItem('API_KEY_OPENAI') || import.meta.env.VITE_OPENAI_KEY)
    return true
  })(configTick)
    ...
    <div style={{ display: 'flex', height: '100vh', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', gap: 6 }}>
              <select className="form-select" style={{ width: 'auto', fontSize: 13 }} value={aiProvider}
                onChange={e => { 
                  const p = e.target.value;
                  setAiProvider(p); 
                  const m = MODELS[p][0];
                  setAiModel(m); 
                  localStorage.setItem('PREF_AI_PROVIDER', p);
                  localStorage.setItem('PREF_AI_MODEL', m);
                }}>
                <option value="claude">Anthropic</option>
                <option value="gemini">Google</option>
                <option value="openai">OpenAI</option>
              </select>
              <select className="form-select" style={{ width: 'auto', fontSize: 13, maxWidth: 140 }} value={aiModel}
                onChange={e => {
                  setAiModel(e.target.value);
                  localStorage.setItem('PREF_AI_MODEL', e.target.value);
                }}>
                {MODELS[aiProvider].map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <button 
              onClick={() => setShowSettings(true)} 
              style={{ background: 'none', border: 'none', color: 'var(--primary)', fontSize: 11, cursor: 'pointer', textAlign: 'left', fontWeight: 600, padding: 0 }}>
          ...
          {posts.length > 0 && posts.some(p => !p.post_text) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer', color: 'var(--gray-600)' }}>
                <input type="checkbox" checked={genWithImage} onChange={e => setGenWithImage(e.target.checked)} />
                Gerar imagens (Freepik/OpenAI)
              </label>
              <button className="btn btn-primary btn-sm" onClick={generateAllPosts} disabled={generatingAll}>
      ...
      {noApiKey && (
        <div className="alert alert-error" style={{ margin: '12px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>⚠️ A chave da API não está configurada para este provedor.</div>
          <button className="btn btn-primary btn-sm" onClick={() => setShowSettings(true)}>Configurar agora</button>
        </div>
      )}
      {aiError && (
        <div className="alert alert-error" style={{ margin: '12px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {showSettings && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div className="card" style={{ width: '100%', maxWidth: 500, background: 'white', padding: 24, borderRadius: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontSize: 18, fontWeight: 700 }}>⚙️ Configurar chaves de API</h2>
              <button onClick={() => { setShowSettings(false); setConfigTick(t => t + 1) }} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer' }}>×</button>
            </div>
            
            <p style={{ fontSize: 13, color: 'var(--gray-500)', marginBottom: 20 }}>
              As chaves salvas aqui substituem as globais e ficam seguras <b>apenas no seu navegador</b>. 
            </p>

            <div style={{ marginBottom: 24 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--gray-700)', marginBottom: 16 }}>Texto (Claude, Google ou OpenAI)</h3>
              {['claude', 'gemini', 'openai'].map(prov => {
                const name = prov === 'claude' ? 'Anthropic (Claude)' : prov === 'gemini' ? 'Google (Gemini)' : 'OpenAI (GPT)';
                const link = prov === 'claude' ? 'https://console.anthropic.com/settings/keys' : prov === 'gemini' ? 'https://aistudio.google.com/app/apikey' : 'https://platform.openai.com/api-keys';
                const lsKey = `API_KEY_${prov.toUpperCase()}`;
                return (
                  <div key={prov} style={{ marginBottom: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <label className="form-label" style={{ marginBottom: 0 }}>{name}</label>
                      <a href={link} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: 'var(--primary)' }}>Criar chave ↗</a>
                    </div>
                    <input 
                      type="password" 
                      className="form-input" 
                      placeholder={localStorage.getItem(lsKey) ? 'Chave configurada ✅' : 'ex: sk-... (deixe em branco para usar .env)'} 
                      onChange={e => {
            ...
            <div style={{ marginBottom: 24 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--gray-700)', marginBottom: 16 }}>Imagem (Freepik)</h3>
              <p style={{ fontSize: 12, color: 'var(--gray-500)', marginBottom: 12, marginTop: -8 }}>Recomendado. Qualidade superior e custo baixo.</p>
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <label className="form-label" style={{ marginBottom: 0 }}>Freepik Mystic (Opcional)</label>
                  <a href="https://www.freepik.com/api/pricing" target="_blank" rel="noreferrer" style={{ fontSize: 11, color: 'var(--primary)' }}>Criar chave ↗</a>
                </div>
                <input 
                  type="password" 
                  className="form-input" 
                  placeholder={localStorage.getItem('API_KEY_FREEPIK') ? 'Chave configurada ✅' : 'ex: ... (se preenchida, substitui OpenAI/Gemini para imagens)'} 
                  onChange={e => {
                    if (e.target.value.trim()) {
                      localStorage.setItem('API_KEY_FREEPIK', e.target.value.trim());
                    } else {
                      localStorage.removeItem('API_KEY_FREEPIK');
                    }
                  }} 
                />
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24 }}>
              <button className="btn btn-primary" onClick={() => { setShowSettings(false); setConfigTick(t => t + 1) }}>
                Pronto
              </button>
            </div>
          </div>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: 24, background: 'var(--gray-50)' }}>
            {editingPost ? (
              ...
              <PostViewer
                ...
                onGenerateImage={() => generatePostImage(selectedPost)}
                onStatusChange={status => updatePostStatus(selectedPost.id, status)}
                generating={generatingPost === selectedPost.id}
                generatingImage={generatingImage === selectedPost.id}
                noApiKey={noApiKey}
                client={client}
                services={services}
                copiedField={copiedField}
                onCopy={copyToClipboard}
                genWithImage={genWithImage}
                setGenWithImage={setGenWithImage}
...
function PostViewer({ post, onEdit, onDelete, onGenerate, onGenerateImage, onStatusChange, generating, generatingImage, noApiKey, client, services, copiedField, onCopy, genWithImage, setGenWithImage, aiProvider, aiModel, dbTemplates }) {
  ...
  return (
    <div>
      ...
      {post.client_feedback && (post.status === 'change_requested' || post.status === 'aprovado') && (
        <div style={{
          background: post.status === 'change_requested' ? '#FEF2F2' : '#F0FDF4',
          borderLeft: `4px solid ${post.status === 'change_requested' ? '#EF4444' : '#22C55E'}`,
          padding: '12px 16px',
          marginBottom: 16,
          color: post.status === 'change_requested' ? '#991B1B' : '#166534',
          borderTopRightRadius: 8,
          borderBottomRightRadius: 8
        }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>
            {post.status === 'change_requested' ? '🚨 Alteração Solicitada pelo Cliente:' : '💬 Comentário de Aprovação:'}
          </div>
          <div style={{ fontSize: 14, whiteSpace: 'pre-wrap' }}>{post.client_feedback}</div>
        </div>
      )}
      ...
      {!post.post_text ? (
        ...
        <>
          ...
          <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer', color: 'var(--gray-600)' }}>
              <input type="checkbox" checked={genWithImage} onChange={e => setGenWithImage(e.target.checked)} />
              Incluir arte (Freepik / IA)
            </label>
            <button className="btn btn-secondary btn-sm" onClick={onGenerate} disabled={generating} style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              {generating ? <><span className="spinner" style={{ width: 12, height: 12 }} />Gerando...</> : <><IconSpark />Regenerar conteúdo</>}
            </button>
            <button className="btn btn-secondary btn-sm" onClick={() => {
              setShowStudio(true);
              setTimeout(() => {
                document.getElementById(`studio-${post.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
              }, 100);
            }} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, color: 'var(--primary)', borderColor: 'var(--primary)' }}>
              <IconSpark /> Abrir Estúdio de Imagem IA
            </button>
          </div>
        </>
      )}

      {/* Galeria Clássica / Estúdio */}
      <div id={`studio-${post.id}`} style={{ marginTop: 24, borderTop: '1px dashed var(--gray-200)', paddingTop: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <h4 style={{ fontSize: 13, fontWeight: 700, color: 'var(--gray-800)', margin: 0 }}>Artes do Post</h4>
          <button 
            className="btn btn-secondary btn-sm" 
            style={{ display: 'flex', alignItems: 'center', gap: 6, opacity: showStudio ? 1 : 0.8, borderColor: showStudio ? 'var(--primary)' : undefined }}
            onClick={() => setShowStudio(!showStudio)}
          >
            <IconSpark /> {showStudio ? 'Voltar para Criador Clássico' : 'Abrir Estúdio de Imagem IA'}
          </button>
        </div>
        ...
        {showStudio ? (
          <AIStudio 
            post={post} 
            client={client} 
            imageProvider={localStorage.getItem('API_KEY_FREEPIK') ? 'freepik' : (aiProvider === 'gemini' ? 'gemini' : 'openai')}
            onSave={async (b64) => {
              try {
                const updates = { image_url: b64, image_prompt: 'Created with AI Studio' }
                await supabase.from('posts').update(updates).eq('id', post.id)
                post.image_url = b64
                setShowStudio(false)
              } catch(e) {
                console.error(e)
                alert('Erro ao salvar imagem final')
              }
            }} 
          />
        ) : (
          <ImageCreator post={post} client={client} services={services} aiProvider={aiProvider} aiModel={aiModel} dbTemplates={dbTemplates} />
        )}
      </div>
    </div>
  )
