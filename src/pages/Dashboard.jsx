      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="btn btn-secondary" onClick={() => {
            navigator.clipboard.writeText(`${window.location.origin}/onboarding/${user.id}`)
            alert('Link copiado! Envie este link para o cliente preencher o formulário.')
          }}>
            📋 Copiar link form
          </button>
          ...
          <div className="table-wrapper">
            {recentClients.length === 0 ? (
              <div className="empty-state">
                ...
                <p>Você pode cadastrar manualmente ou pedir ao cliente para preencher.</p>
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 16 }}>
                  <button className="btn btn-secondary" onClick={() => {
                    navigator.clipboard.writeText(`${window.location.origin}/onboarding/${user.id}`)
                    alert('Link copiado! Envie para o seu cliente preencher.')
                  }}>
                    📋 Copiar link form
                  </button>
                  <button className="btn btn-primary" onClick={() => navigate('/clientes/novo')}>
